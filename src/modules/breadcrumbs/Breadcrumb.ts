...
const getBreadcrumb = (route: Route<typeof routes>, organization?: Organization): BreadcrumbProps | undefined => {
    ...
    const organizationProp: BreadcrumbProps = {
        homeLinkProps: routes.home().link,
        // segments: [{ label: t("dashboard"), linkProps: routes.dashboard().link }],
        segments: [...organizationBaseProp.segments, { label: t("organization_list"), linkProps: routes.organization_list().link }, organizationSegment].filter(
            Boolean
        ) as BreadcrumbProps["segments"],
        currentPageLabel: t("organization_list"),
    };
    ...
    switch (route.name) {
        ...
        case "organization_maps":
            return {
                ...organizationProp,
                segments: [...organizationBaseProp.segments, { label: t("organization_list"), linkProps: routes.organization_list().link }],
